 ...
 * - In Wicket mode: press a run (0–6) -> runs are added AND wicket recorded on
 * the SAME delivery.
 * Then the popup opens to pick who is out and helper/catcher. One DB row is
...
public class LiveScoreUI extends JFrame {
    ...
    private String teamBatting = "Batting Team";
    private String teamBowling = "Bowling Team";
    ...
    private static class BatStat {
        int r, b, f, s;
        String out = "not out";
    }

    private static class BowlStat {
        int balls, maidens, runs, wkts;
    ...
    private JLabel lblScore, lblCRR, lblRRR, lblChaseLeft, lblStatus, lblPrediction;
    ...
    public LiveScoreUI(String striker, String nonStriker, String bowler, int totalOvers, boolean firstInnings,
            int target) {
    ...
    private void initUI() {
        ...
        lblPrediction = label("Predictor: --", new Color(255, 223, 0), new Font("Segoe UI", Font.BOLD, 15));
        ...
        // Changed to 2 rows x 4 cols so we can add a Scorecard button after the '6'
        // button
    ...
    private JButton primaryButton(String txt) {
        return baseButton(txt, ACCENT, ACCENT_HOVER);
    }

    private JButton neutralButton(String txt) {
        return baseButton(txt, new Color(90, 100, 125), new Color(110, 130, 150));
    }

    private JButton dangerButton(String txt) {
        return baseButton(txt, new Color(220, 66, 66), new Color(235, 85, 85));
    }

    private JButton warnButton(String txt) {
        return baseButton(txt, new Color(160, 82, 45), new Color(185, 100, 60));
    }
    ...
    private void setMode(Mode m) {
        ...
        btnWide.setBorder(
                BorderFactory.createLineBorder(mode == Mode.WIDE ? on : new Color(255, 255, 255, 120), 2, true));
        btnNoBall.setBorder(
                BorderFactory.createLineBorder(mode == Mode.NOBALL ? on : new Color(255, 255, 255, 120), 2, true));

        // Turning on any other mode disables wicket mode
        if (mode != Mode.NORMAL && wicketMode)
            toggleWicketModeOff();
    ...
    private void toggleWicketMode() {
        ...
        if (wicketMode && mode != Mode.NORMAL)
            mode = Mode.NORMAL;
        // Visual cue on button border
        btnWicket.setBorder(
                BorderFactory.createLineBorder(wicketMode ? Color.GREEN : new Color(255, 255, 255, 120), 2, true));
    ...
    private void addRun(int run) {
        ...
        if (bw == null) {
            bw = new BowlStat();
            bowling.put(bowler, bw);
        }
        bw.balls++;
        bw.runs += run;
        bw.runsInCurrentOver += run;
        ...
        DB.upsertBatting(MatchContext.sessionId, inningsNo, striker, bs.r, bs.b, bs.f, bs.s, bs.out);
        // Also ensure non-striker present in batting_scorecard (keep their values)
        BatStat ns = batting.get(nonStriker);
        if (ns != null)
            DB.upsertBatting(MatchContext.sessionId, inningsNo, nonStriker, ns.r, ns.b, ns.f, ns.s, ns.out);

        DB.upsertBowling(MatchContext.sessionId, inningsNo, bowler, bw.balls, bw.maidens, bw.runs, bw.wkts);
        // ----------------------------------------------------------------------
    ...
    private void addRunThenWicket(int run) {
        ...
        // 1) Add the runs as a normal ball (BUT delay DB write until after popup so we
        // can write single row)
        runs += run;
        balls++;
        ...
        // Save current batting positions (after potential swap) — these are the
        // PRE-REPLACEMENT names
        ...
        if (outWasPrevStriker) {
            ...
            strikerRuns = newBs.r;
            strikerBalls = newBs.b;
            strikerFours = newBs.f;
            strikerSixes = newBs.s;
            ...
            if (otherBs != null) {
                ...
                nonStrikerRuns = otherBs.r;
                nonStrikerBalls = otherBs.b;
                nonStrikerFours = otherBs.f;
                nonStrikerSixes = otherBs.s;
            }
        } else {
            // prevNonStriker out -> nonStriker becomes newBat
            nonStriker = newBat;
            nonStrikerRuns = newBs.r;
            nonStrikerBalls = newBs.b;
            nonStrikerFours = newBs.f;
            nonStrikerSixes = newBs.s;

            // other remains prevStriker; ensure its stat variables reflect batting map
            BatStat otherBs = batting.get(prevStriker);
            if (otherBs != null) {
                striker = prevStriker;
                strikerRuns = otherBs.r;
                strikerBalls = otherBs.b;
                strikerFours = otherBs.f;
                strikerSixes = otherBs.s;
            }
        }
        ...
        DB.upsertBatting(MatchContext.sessionId, inningsNo, whoOut, outBat.r, outBat.b, outBat.f, outBat.s, outBat.out);
        // Upsert new batsman (ensure present with values)
        DB.upsertBatting(MatchContext.sessionId, inningsNo, newBat, newBs.r, newBs.b, newBs.f, newBs.s, newBs.out);
        ...
        if (otherPreBs != null) {
            DB.upsertBatting(MatchContext.sessionId, inningsNo, otherPre, otherPreBs.r, otherPreBs.b, otherPreBs.f, otherPreBs.s,
                    otherPreBs.out);
        }

        DB.upsertBowling(MatchContext.sessionId, inningsNo, bowler, bow.balls, bow.maidens, bow.runs, bow.wkts);
        // ------------------------------------------------------------------
    ...
    private void addWideWith(int x) {
        ...
        // --- NEW: upsert bowling (wide affects bowler runs) and ensure batsmen exist
        // in batting_scorecard
        DB.upsertBowling(MatchContext.sessionId, inningsNo, bowler, bw.balls, bw.maidens, bw.runs, bw.wkts);
        BatStat bs = batting.get(striker);
        if (bs != null)
            DB.upsertBatting(MatchContext.sessionId, inningsNo, striker, bs.r, bs.b, bs.f, bs.s, bs.out);
    ...
    private void addNoBallWith(int x) {
        ...
        // --- NEW: upsert bowling and potentially batting if batsman got runs from
        // no-ball extras
        DB.upsertBowling(MatchContext.sessionId, inningsNo, bowler, bw.balls, bw.maidens, bw.runs, bw.wkts);
        BatStat bs = batting.get(striker);
        if (bs != null)
    ...
    private String makeOutText(String type, String helpedBy) {
        String clean = type;
        if ("Run out".equals(type))
            clean = "run out";
        if ("Catch out".equals(type))
            clean = "c";
        if ("Bowled".equals(type))
            clean = "b";
        if ("Stumping".equals(type))
            clean = "st";
        if ("LBW".equals(type))
            clean = "lbw";
        if ("Hit wicket".equals(type))
            clean = "hit wicket";
     ...
     * - ONLY IF innings still ongoing prompt for new bowler and FORCE typing
     * (Option B).
    ...
    private void checkOverCompletion() {
        ...
        // 5) If innings is still ongoing, prompt for next bowler and FORCE input
        // (Option B)
        if (!isInningsComplete()) {
    ...
    private void updateScoreboard(String msg) {
        ...
        lblPrediction.setText(getWinPrediction());
    ...
    private void renderThisOver() {
        ...
        for (String s : thisOverEntries) {
            if (count > 0)
                sb.append(", "); // commas only BETWEEN separate deliveries
            sb.append(s);
            count++;
            if (count % 10 == 0)
                sb.append("<br>");
    ...
    private void flushOpenPlayersAsNotOut() {
        ...
        if (s != null)
            DB.upsertBatting(MatchContext.sessionId, inningsNo, striker, s.r, s.b, s.f, s.s, s.out);

        BatStat n = batting.get(nonStriker);
        if (n != null)
            DB.upsertBatting(MatchContext.sessionId, inningsNo, nonStriker, n.r, n.b, n.f, n.s, n.out);

        for (Map.Entry<String, BatStat> e : batting.entrySet()) {
            DB.upsertBatting(MatchContext.sessionId, inningsNo, e.getKey(), e.getValue().r, e.getValue().b,
    ...
    private void flushCurrentBowler() {
        for (Map.Entry<String, BowlStat> e : bowling.entrySet()) {
            DB.upsertBowling(MatchContext.sessionId, inningsNo, e.getKey(), e.getValue().balls,
    ...
    private Action snapshot(String type) {
        ...
        a.prevStrikerRuns = strikerRuns;
        a.prevStrikerBalls = strikerBalls;
        a.prevStrikerFours = strikerFours;
        a.prevStrikerSixes = strikerSixes;
        a.prevNonStrikerRuns = nonStrikerRuns;
        a.prevNonStrikerBalls = nonStrikerBalls;
        a.prevNonStrikerFours = nonStrikerFours;
        a.prevNonStrikerSixes = nonStrikerSixes;

        a.prevBowlerBalls = bowlerBalls;
        a.prevBowlerRuns = bowlerRuns;
        a.prevBowlerWkts = bowlerWickets;
        a.prevBowlerMaidens = bowlerMaidens;
        a.prevRunsThisOver = runsThisOver;
        a.prevUiBallsInOver = uiBallsInOver;
        return a;
    }
    ...
    private void restoreFromAction(Action a) {
        ...
        wickets = a.prevWkts;
        balls = a.prevBalls;
        striker = a.prevStriker;
        nonStriker = a.prevNonStriker;
        bowler = a.prevBowler;

        strikerRuns = a.prevStrikerRuns;
        strikerBalls = a.prevStrikerBalls;
        strikerFours = a.prevStrikerFours;
        strikerSixes = a.prevStrikerSixes;
        nonStrikerRuns = a.prevNonStrikerRuns;
        nonStrikerBalls = a.prevNonStrikerBalls;
        nonStrikerFours = a.prevNonStrikerFours;
        nonStrikerSixes = a.prevNonStrikerSixes;

        bowlerBalls = a.prevBowlerBalls;
        bowlerRuns = a.prevBowlerRuns;
        bowlerWickets = a.prevBowlerWkts;
        bowlerMaidens = a.prevBowlerMaidens;
        runsThisOver = a.prevRunsThisOver;
        uiBallsInOver = a.prevUiBallsInOver;
        popOverIcon(a);
    }
    ...
    // ======================== UTILS ===============================
    private String sr(int r, int b) {
        return b == 0 ? "0.00" : String.format("%.2f", r * 100.0 / b);
    }

    private String er(int r, int b) {
        return b == 0 ? "0.00" : String.format("%.2f", r * 6.0 / b);
    }

    private String toOvers(int b) {
        return (b / 6) + "." + (b % 6);
    }
    ...
    private void loadTeamNames() {
        String sql = "SELECT team1, team2, toss_winner, opted FROM toss_details ORDER BY id DESC LIMIT 1";
        try (Connection c = DB.get(); PreparedStatement ps = c.prepareStatement(sql)) {
            java.sql.ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                String t1 = rs.getString("team1");
                String t2 = rs.getString("team2");
                String winner = rs.getString("toss_winner");
                String opted = rs.getString("opted");

                String bat = winner;
                String bowl = t1.equals(winner) ? t2 : t1;

                if ("Bowl".equalsIgnoreCase(opted)) {
                    bat = t1.equals(winner) ? t2 : t1;
                    bowl = winner;
                }
    ...
    private String getWinPrediction() {
        int totalBalls = totalOvers * 6;
        if (firstInnings) {
            double projScore = (balls == 0) ? 0 : ((double) runs / balls) * totalBalls;
            int parScore = totalOvers * 8; // par 8 runs per over
            double diff = projScore - parScore;
            double winProb = 50.0 + (diff * 0.2) - (wickets * 3.0);
            if (winProb > 95)
                winProb = 95;
            if (winProb < 5)
                winProb = 5;
            return String.format("Predictor: %s %.0f%% | %s %.0f%%", teamBatting, winProb, teamBowling, 100 - winProb);
        } else {
            if (runs >= target)
                return "Predictor: " + teamBatting + " Won";
            int runsNeeded = target - runs;
            int ballsLeft = totalBalls - balls;
            if (ballsLeft <= 0)
                return "Predictor: " + teamBowling + " Won";
            double rrr = ((double) runsNeeded / ballsLeft) * 6.0;
            double crr = (balls == 0) ? 0 : ((double) runs / balls) * 6.0;
            double winProb = 50.0 + ((crr - rrr) * 5.0) + ((10 - wickets) * 2.5);
            if (winProb > 95)
                winProb = 95;
            if (winProb < 5)
                winProb = 5;
    ...
    private static class WicketPopup extends JDialog {
        ...
        public String getNewBatsman() {
            return newBatsman;
        }

        public String getWhoOutName() {
            return whoOutName;
        }
